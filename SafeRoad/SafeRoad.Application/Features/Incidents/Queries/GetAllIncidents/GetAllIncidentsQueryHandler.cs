...
public class GetAllIncidentsQueryHandler : IRequestHandler<GetAllIncidentsQuery, ApiResponse<List<IncidentResponse>>>
{
    ...
    public async Task<ApiResponse<List<IncidentResponse>>> Handle(GetAllIncidentsQuery request, CancellationToken cancellationToken)
    {
        var incidents = await _incidentRepository.GetAllPaginatedAsync(request.Page, request.PageSize);
